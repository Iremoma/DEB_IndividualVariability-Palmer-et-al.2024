...
library(ggplot2) 
library(cmdstanr)
fit = readRDS("out.RDS") # results from the analysis in the main text
draws=fit$draws(format="matrix")
load("input_S8.RData") # simulated data (produced at "S8_Rich.R")
...
# 2.1: STAN model
#----
# Note that this model is the same as in "S8_Rich.R" but likelihood
#  is estimated from length and weight only.
sink("DEB.stan")
cat(" // first line
functions {
  // DEB function and Oxygen function
  //////////////////////////////
  // DEB
  vector DEB(
    real t,          // time
...
# 2.6: saving
#-----
# fit$save_object(file = "S8_poor.RDS")
# sink(file = "dignosi_S8_poor.txt")
# fit$cmdstan_diagnose()
# sink()
# file.rename("DEB.stan","model_S8_poor.R")
...
#-----
# 3.1: Re-loading data and libraries
#-----
remove(list=ls())
library(cmdstanr)
library(bayesplot) 
...
library(ggplot2)
load("input_S8.RData")
fit = readRDS("S8_poor.RDS")
draws=fit$draws(format="matrix")

#-----
# 3.2: Convergence quality control (parameter specific checking)
#-----
...
#-----
# 3.4: Re-setting priors (as defined for the analysis)
#-----
...
#-----
# Figure S10: Prior (blue), posterior distribution (red) and true values 
# (vertical dashed line) for the DEB parameters estimated in a data poor scenario 
# (length and weight). Note that in this case, precision of the estimates (posterior 
# distribution) is worse for all the parameters. Moreover, in many cases, posterior
# distributions are very close to prior distribution, which suggests estimation problems.
#----- 
